...
class UnifiedToolHandler:
    ...
    def __init__(self, conn, device_id: str = None, secret_id: str = None, secret_key: str = None):
        ...
        # 创建各类执行器，为MCP执行器传入设备ID和API认证信息
        self.server_plugin_executor = ServerPluginExecutor(conn)
        self.server_mcp_executor = ServerMCPExecutor(conn, device_id, secret_id, secret_key)
    ...
    async def _initialize(self):
        ...
        try:
            ...
            # 初始化服务端MCP
            self.logger.info("正在初始化服务端MCP...")
            await self.server_mcp_executor.initialize()
            self.logger.info("服务端MCP初始化完成")
            ...
            # 强制刷新工具列表缓存
            self.tool_manager._invalidate_cache()
