...
class ToolManager:
    ...
    def get_all_tools(self) -> Dict[str, ToolDefinition]:
        ...
        if self._cached_tools is not None:
            self.logger.debug(f"从缓存返回 {len(self._cached_tools)} 个工具")
            return self._cached_tools

        all_tools = {}
        self.logger.info(f"重新收集工具，当前注册的执行器: {list(self.executors.keys())}")

        for tool_type, executor in self.executors.items():
            try:
                self.logger.info(f"从 {tool_type.value} 执行器获取工具...")
                tools = executor.get_tools()
                self.logger.info(f"{tool_type.value} 执行器返回 {len(tools)} 个工具: {list(tools.keys())}")
        ...
        self.logger.info(f"工具收集完成，总计 {len(all_tools)} 个工具: {list(all_tools.keys())}")
        return all_tools
